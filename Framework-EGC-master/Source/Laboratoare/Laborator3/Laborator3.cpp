...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	modelMatrix *= Transform2D::Translate(325 + translateX, 300 + translateY) * Transform2D::Rotate(angularStep) * Transform2D::Translate(-250, -50);
	// TODO: create animations by multiplying current transform matrix with matrices from Transform 2D

	RenderMesh2D(meshes["square1"], shaders["VertexColor"], modelMatrix);

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(275 + translateX, 250 + translateY);
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
...
void Laborator3::OnInputUpdate(float deltaTime, int mods)
{
	if (go)
	{
		...
		translateX += 350 * deltaTime * reverse;

		if (translateX >= 900 || translateX <= -275) {
			reverse = (-1) * reverse;
		}

		if (scaleX >= 4 || scaleX <= 0.1) {
